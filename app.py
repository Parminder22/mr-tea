...
/* ── NATIVE FIXED CONTROLS COMPONENT (LEFT SIDE CARD) ── */
...
/* BRANDING CARD (FIXED BACKGROUND & CONTRAST) */
.sb-brand-card {
    background: #F8FAFC;
...
.sb-tagline { font-size: 11px; color: #6B7280; margin-bottom: 10px; font-weight: 500; }
...
.sb-section { font-size: 10px !important; font-weight: 800 !important; letter-spacing: 0.1em !important;
              text-transform: uppercase !important; color: #6B7280 !important;
              padding: 12px 0 6px 0 !important; border-top: 1px solid #F3F4F6 !important;
              margin-top: 6px !important; display: block; }

/* ── ABSOLUTE FORCE BLACK TEXT FOR ALL SLIDER LABELS ── */
...
.stSlider div[data-testid="stSliderTickBarMax"] {
    color: #000000 !important; /* Pitch black for min/max labels */
...
/* ── KPI CARDS GRID (STRICT 4-COLUMN ENFORCEMENT) ── */
.kpi-grid { 
    display: grid; 
...
def chart_card(title, subtitle, fig, key=None):
    fig.update_layout(
        title=dict(
            text=f"<span style='font-size:13px; font-weight:800; color:#000000'>{title}</span><br><span style='font-size:11px; color:#6B7280; font-weight:400'>{subtitle}</span>",
...
with left_panel:
    # This st.container(border=True) creates the native white card around widgets
    with st.container(border=True):
        ...
        daily_orders = st.slider("Daily Orders",          30,  300, 125, 5, key="inp_orders")
        ticket_size  = st.slider("Avg Ticket Size (₹)",  250,  600, 400, 10, key="inp_ticket")

        st.markdown('<span class="sb-section">🧪 Cost Per Cup</span>', unsafe_allow_html=True)
        raw_mat     = st.slider("Raw Material / Cup (₹)",  40, 150, 100, 5, key="inp_rm")
        pkg_cost    = st.slider("Packaging / Cup (₹)",     10,  40,  25, 1, key="inp_pkg")
        wastage_pct = st.slider("Wastage Buffer (%)",        0,  10,   5, 1, key="inp_wst")
        royalty_pct = st.slider("Franchise Royalty (%)",     0,  15,   5, 1, key="inp_roy")

        st.markdown('<span class="sb-section">🏢 Monthly Fixed Costs</span>', unsafe_allow_html=True)
        rent_k    = st.slider("Rent (₹ thousands)",       100, 400, 250, 10, key="inp_rent")
        payroll_k = st.slider("Staff Payroll (₹ thou.)",   30, 150,  60,  5, key="inp_pay")
        utils_k   = st.slider("Utilities & Misc (₹ th.)",  10,  50,  20,  2, key="inp_utils")
...
with right_panel:
    ...
    <div class="page-header">
      <div>
        ...
        <div class="page-header-sub">Mr. Tea · New Delhi · Model performance vectors in real-time</div>
      </div>
    ...
    st.markdown(f"""<div class="kpi-grid">
      ...
      {kpi("Daily Break-even",    f"{r['be']:.0f} cups",f"{be_ratio*100:.0f}% of target", be_b, b_cls)}
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        chart_card("12-Month Revenue & Profit Trajectory",
                   "Assumes 3% month-over-month order volume compounding growth", fig1)
    ...
    with col2:
        ...
        chart_card("Cost Distribution Matrix", "Monthly opex breakdown structured by operational footprint", fig2)
    ...
    with col3:
        ...
        chart_card("Monthly P&L Waterfall View", "Gross execution down to final net earnings", fig3)
    ...
    with col4:
        ...
        chart_card("Per-Cup Unit Economics", "Resource allocation vs net value extraction", fig4)
    ...
    chart_card("Sensitivity Heatmap — Net Profit (₹ Lakh)",
               "× = current structural position • green = high profit margin • red = down side", fig5)

    # ══════════════════════════════════════════════════════
    # ROW 5 — Scenario table
    # ══════════════════════════════════════════════════════
    rows_html = ""
    ...
    <div class="sc-wrap">
      ...
      <div class="sc-sub">Highlighted row = current simulated state • cross-referenced constraints held stable</div>
